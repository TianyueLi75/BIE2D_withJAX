...
UOMexact = [];
utexact = [];
err_UO = zeros(numel(Nord_lst),1);
err_ut = zeros(numel(Nord_lst),1);
for ind=numel(Nord_lst):-1:1
    Num_panels = Nord_lst(ind);
    ...
    if ind==numel(Nord_lst)
        % Exact solution
        UOMexact = [U;Omega_z];
        Utexact = ux;
    else
        % compare with exact
        err_UO(ind) = norm(UOMexact - [U;Omega_z]);
        err_ut_temp = abs(ux - Utexact);
        err_ut(ind) = norm(err_ut_temp - mean(err_ut_temp));
    end
...
figure;
loglog(p.*Nord_lst,err_UO); hold on;
loglog(p.*Nord_lst,err_ut); hold off;
legend("U,Omega err", "ut err");
