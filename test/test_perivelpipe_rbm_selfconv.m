...
Utexact = [];
ptexact = [];
err_UO = zeros(numel(Num_panels),1);
err_ut = zeros(numel(Num_panels),1);
err_pt = zeros(numel(Num_panels),1);
Nptcl_lst = zeros(numel(Num_panels),1);
for ind=numel(Num_panels):-1:1
    npanel = Num_panels(ind);
    N_perwall = p*npanel;
    Nptcl = N_perwall;
    % Nptcl = 280;
    Nptcl_lst(ind) = Nptcl;
    % N_perwall = p*20; % refix channel discretization for ptcl self conv.
    ...
    if ind==numel(Num_panels)
        % Exact solution
        UOMexact = [U_ptcl_all;Omega_ptcl_all];
        Utexact = ut;
        ptexact = pt;
    else
        % compare with exact
        err_UO(ind) = norm(UOMexact - [U_ptcl_all;Omega_ptcl_all]);
        err_ut(ind) = norm(ut - Utexact);
        % Normalize pt error
        pt_err = ptexact - pt;
        pt_err = pt_err - mean(pt_err);
        err_pt(ind) = norm(pt_err);
    end
...
% figure;
% % plot(Nptcl_lst,log10(err_UO)); hold on;
% % plot(Nptcl_lst,log10(err_ut));
% % plot(Nptcl_lst,log10(err_pt)); hold off;
% plot(p.*Num_panels,log10(err_UO)); hold on;
% plot(p.*Num_panels,log10(err_ut));
% plot(p.*Num_panels,log10(err_pt)); hold off;
% legend("U,Omega err", "ut err", "pt err");

figure;
loglog(p.*Num_panels,err_UO); hold on;
loglog(p.*Num_panels,err_ut);
loglog(p.*Num_panels,err_pt); hold off;
legend("U,Omega err", "ut err", "pt err");
