...
def srcsum_ptcl_stodl_closeglobal(peri_len, tx, tnx, ptx, ptnx, pta, ptxp, ptw, ptwxp, sigma, mu):
    ...
    ptx1 = ptx - peri_len
    [u,p,_] = StoDLP(tx, tnx, ptx1, ptnx, ptw, mu, sigma)

    # right copy, also should be far.
    ptx1 = ptx + peri_len
    [tempu,tempp,_] = StoDLP(tx, tnx, ptx1, ptnx, ptw, mu, sigma)
    u = u + tempu
    p = p + tempp
...
def srcsum_wall_closepanel(peri_len, tx, tnx, sx, sxlo, sxhi, snx, sxp, scur, sws, mu):
    ...
    # first translation
    sx1 = sx - peri_len
    [u,p,_] = StoDLP(tx, tnx, sx1, snx, sws, mu, jnp.array([]))

    # Hardcode trlist 3 entries, L, middle, R
    sigma_real = jnp.eye(2*sx.shape[0])
    [tempu,tempp] = stoDLP_closepanel(tx, tnx, sx, sxp, sws, scur, sxlo, sxhi, sigma_real, mu)
    u = u + tempu
    p = p + tempp

    sx1 = sx + peri_len
    [tempu,tempp,_] = StoDLP(tx, tnx, sx1, snx, sws, mu, jnp.array([]))
    u = u + tempu
    p = p + tempp
...
def evalsol_panel(tx, tnx, sx, sxlo, sxhi, snx, sxp, scur, sw, px, pwt, peri_len, mu, edens):
    ...
    [u, p, _] = StoSLP(tx, tnx, px, pwt, mu, prx_dens)
    [uD, pD] = srcsum_wall_closepanel(peri_len, tx, tnx, sx, sxlo, sxhi, snx, sxp, scur, sw, mu)
    uD = uD @ wall_dens
    pD = pD @ wall_dens
...
def evalsol_stokeslet(tx, tnx, sx, sxlo, sxhi, snx, sxp, scur, sw, x_stokeslet, w_stokeslet, px, pwt, peri_len, mu, edens):
    ...
    [u, p, _] = StoSLP(tx, tnx, px, pwt, mu, prx_dens)
    [uD, pD] = srcsum_wall_closepanel(peri_len, tx, tnx, sx, sxlo, sxhi, snx, sxp, scur, sw, mu) # no need for DL traction so fill in tnx with snx
    x_stokeslet_near = x_stokeslet + trlist[0] # left copy first
